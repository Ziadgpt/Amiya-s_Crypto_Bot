...
try:
    ...
    if df.empty or 'started_at' not in df.columns:
        logger.warning(f"No data or missing 'started_at' in SQLite for {market}. Table info: {pd.read_sql(f'PRAGMA table_info({table_name})', sqlite3.connect('crypto_data.db'))}")
...
if not df.empty and 'started_at' in df.columns:
    ...
    st.line_chart(df_for_plotting[['open', 'close']], height=300, use_container_width=True)
    st.line_chart(df_for_plotting[['volatility']], height=200, use_container_width=True)
    try:
        conn = sqlite3.connect('crypto_data.db')
        available_columns = pd.read_sql(f"PRAGMA table_info({table_name})", conn)['name'].tolist()
        signal_columns = [col for col in ['f7_signal', 'regime', 'hmm_signal'] if col in available_columns]
        if signal_columns:
            ...
            logger.warning(f"No signal columns (f7_signal, regime, hmm_signal) in {table_name}")
    ...
    logger.error(f"Dashboard render failed: No valid data with 'started_at' for {market}. Columns: {df.columns.tolist() if not df.empty else 'Empty'}")
